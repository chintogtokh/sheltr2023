...
rankedSuburbRouter.post("/", (req, res) => {
  ...
  var language =
    typeof preferences.language !== "undefined" ? preferences.language : null;
  var uni = typeof preferences.uni !== "undefined" ? preferences.uni : null;
  var filter =
    typeof preferences.filter !== "undefined" ? preferences.filter : null;
  var distance =
    typeof preferences.distance !== "undefined" ? preferences.distance : null;
  ...
  //distance
  query[0]["$match"]["university_distances." + uni + ".number"] = {
    $exists: true,
    $lt: parseFloat(distance),
  };

  //remove population zero
  query[0]["$match"]["stats.suburb-residents.number"] = {
    $exists: true,
    $ne: 0,
  };

  //coz data
  query[0]["$match"]["stats.suburb-most-common-expense-tier.number"] = {
    $exists: true,
    $ne: "As at 2016, there are no rental properties listed in this area.",
  };
